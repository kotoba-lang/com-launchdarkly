(ns launchdarkly.main
  "Kotodama WASM entrypoint for the Launchdarkly clean-room actor (L4) — Clojure port.
  ...
  facts (`launchdarkly.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "launchdarkly")
(def tier "L4")
...
(def entity-specs
  [{:entity "FeatureFlag" :plural "featureflags" :id-prefix "launchda_fea"
    :fields [:key :name :kind :description :temporary :archived] :required [:key :name]
    :coerce {:temporary :bool :archived :bool} :refs {}}
   {:entity "Project" :plural "projects" :id-prefix "launchda_pro"
    :fields [:key :name :includeInSnippetByDefault] :required [:key]
    :coerce {:includeInSnippetByDefault :bool} :refs {}}
   {:entity "Environment" :plural "environments" :id-prefix "launchda_env"
    :fields [:key :name :apiKey :mobileKey :color] :required [:key]
    :coerce {} :refs {}}
   {:entity "Segment" :plural "segments" :id-prefix "launchda_seg"
    :fields [:key :name :environmentKey :description :deleted :creationDate] :required [:key :name]
    :coerce {:deleted :bool :creationDate :int} :refs {}}
   {:entity "Member" :plural "members" :id-prefix "launchda_mem"
    :fields [:email :firstName :lastName :role] :required [:email]
    :coerce {} :refs {}}
   {:entity "Webhook" :plural "webhooks" :id-prefix "launchda_web"
    :fields [:name :url :on] :required [:name]
    :coerce {:on :bool} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"launchdarkly.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "launchdarkly-compat" :tier tier :entities entities} 200])
...
(defn start! [] :launchdarkly-compat/ready)
